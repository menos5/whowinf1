...
.controller("Drivers", function($scope, $http) {
...
.controller("Constructors", function($scope, $http) {
  $scope.myData =  $http.get("http://ergast.com/api/f1/current/constructorStandings.json").
    success(function(data, status, headers, config) {
      $scope.resultados = data.MRData.StandingsTable.StandingsLists[0].ConstructorStandings;
      $scope.round = data.MRData.StandingsTable.StandingsLists[0].round;
      $scope.carreras = 19;
      $scope.primero = data.MRData.StandingsTable.StandingsLists[0].ConstructorStandings[0].points;

      $scope.prob = function(puntos) {
        if ( (($scope.carreras - $scope.round) * 43) < ($scope.primero - puntos) ) {
          return "eliminado";
        }
      };
      ...
      $scope.b = function(puntos) {
        var posibles = ($scope.carreras - $scope.round) * 43;
        var d =  (parseInt(puntos) + posibles ) / $scope.c();
        return d;
      }

      $scope.c = function() {
        var d =  ($scope.carreras * 43 / 100);
        return d;
      }
